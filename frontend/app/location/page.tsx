...
export const metadata: Metadata = {
  ...
  description: "زورونا فالمحل ديالنا فمدينة أولاد تايمة. جودة عالمية، روح مغربية. العنوان، ساعات العمل، و الطريق.",
};

const storeInfo = [
  { icon: "📍", label: "العنوان", value: "بلوك ب، آيت إيعزة 83000" },
  { icon: "🕐", label: "ساعات العمل", value: "السبت - الخميس: 9:00 صباحاً - 11:00 مساءً\nالجمعة: 3:00 مساءً - 11:00 مساءً" },
  { icon: "📞", label: "الهاتف", value: "0600294302" },
  { icon: "⭐", label: "الخدمات", value: "تجربة المنتجات قبل الشراء · تبديل و إرجاع فالمحل · نصائح ستايل · طلب منتجات خاصة" },
];
    ...
    <div className="pt-14 md:pt-16">
      ...
      <section className="bg-gradient-to-br from-brand-black via-brand-black-light to-brand-black py-12 md:py-20">
        <div className="max-w-7xl mx-auto px-4 text-center">
          ...
          <h1 className="text-2xl md:text-5xl font-bold text-white mb-3 md:mb-4">موقع Smail Store</h1>
          <p className="text-sm md:text-lg text-gray-400 max-w-xl mx-auto">
            جودة عالمية، روح مغربية. تعال جرب المنتجات بنفسك فالمحل ديالنا فأولاد تايمة
          </p>
        </div>
      </section>
      ...
      <section className="max-w-7xl mx-auto px-4 -mt-6 md:-mt-10 relative z-10 pb-12 md:pb-20">
        <div className="bg-white rounded-2xl md:rounded-3xl shadow-premium overflow-hidden border border-border/40">
          {/* Map */}
          <div className="relative w-full group" style={{ paddingBottom: "min(450px, 56.25%)" }}>
            <iframe
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-px bg-border/40">
            {storeInfo.map((item) => (
              <div key={item.label} className="p-5 md:p-7 bg-white">
                <div className="flex items-start gap-4">
                  <span className="text-xl md:text-2xl flex-shrink-0 mt-0.5">{item.icon}</span>
                  <div>
                    <h3 className="font-bold text-sm md:text-base mb-1">{item.label}</h3>
                    <p className="text-xs md:text-sm text-text-secondary leading-relaxed whitespace-pre-line">{item.value}</p>
                  </div>
                </div>
              </div>
            ))}
        ...
        <div className="mt-8 md:mt-10 text-center">
          <p className="text-xs md:text-sm text-text-secondary mb-4 max-w-md mx-auto">
            تقدر تجي للمحل، تجرب المنتجات، و تشوف الجودة بنفسك قبل ما تشري. خدمة الستريوير عندنا فالمغرب
          </p>
